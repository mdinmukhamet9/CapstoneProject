...
int my_NMPC_solver::solve_my_mpc(double current_robot_position[3], double costmap_data[520], 
    ...
    double p[NP];
    for (int i = 0; i < 520; i++) { // changed from NP to 520
        p[i] = costmap_data[i];
    }
    if (person_detected){
        p[520] = person_pos[0];
        ...
        p[520] = 1000.0; // Far away if no person
        p[521] = 1000.0;
    }
    ...
    for(int stage = 0; stage < N; stage++) {
        ...
        const double r_wheel = 0.1;
        const double alpha_human = 1.1;
        

        // Static constraint (index 260)
        double dx_human = x_pred[0] - person_pos[0];
        double dy_human = x_pred[1] - person_pos[1];
        double d_sq_human = dx_human * dx_human + dy_human * dy_human;
        double min_distance_sq_human = (r_base + r_human) * (r_base + r_human);

        if (person_detected) {
            // Static constraint
            if (min_distance_sq_human - d_sq_human > 0) {
                lsh[260] = -1e8;
                ush[260] = 0.0;
            } else {
                lsh[260] = 0.0;
            ...
            // Dynamic constraint (SSM) (index 261)
            ...
            double linear_vel = (0.5 * r_wheel) * (phi_dot_1 + phi_dot_2);
            double v_sq = linear_vel * linear_vel;

            double ssm_value = v_sq - alpha_human*alpha_human * (d_sq_human - min_distance_sq_human);
            if (ssm_value > 0) {
                lsh[261] = -1e8;
                ush[261] = 0.0;
                printf("Stage %d: SSM constraint violated, velocity too high (v_sq=%.3f, max_v_sq=%.3f)\n", 
                    stage, v_sq, alpha_human * alpha_human * (d_sq_human - min_distance_sq_human));
            } else {
                lsh[261] = 0.0;
                ush[261] = 0.0;
            }
        } else {
            // No human - disable constraints
            lsh[260] = 0.0;
            ush[260] = 0.0;
            lsh[261] = 0.0;
            ush[261] = 0.0;
