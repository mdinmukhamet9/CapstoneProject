...
int my_NMPC_solver::solve_my_mpc(double current_robot_position[3], double costmap_data[520], 
    ...
    // Calculate yaw difference to determine turn direction
    double current_yaw = current_robot_position[2];
    double desired_yaw = current_robot_goal[2];
    double yaw_diff = desired_yaw - current_yaw;
    yaw_diff = atan2(sin(yaw_diff), cos(yaw_diff)); // Ensure shortest path
    printf("Yaw difference: %.3f rad (positive = left, negative = right)\n", yaw_diff);
    ...
    double lbu[2] = {-6.0, -6.0};
    double ubu[2] = {6.0, 6.0};

    // double y_ref[NY] = {0.0};
    // y_ref[0] = absolute_goal[0];
    // y_ref[1] = absolute_goal[1];
    // y_ref[2] = absolute_goal[2];
    // y_ref[3] = 0.5; // Encourage positive linear velocity
    // y_ref[4] = yaw_diff > 0 ? 1.0 : -1.0; // Reduced angular velocity reference to improve convergence
    ...
    // printf("Angular velocity reference: %.3f\n", y_ref[4]);

    // double W[(NU+NX)*(NU+NX)] = {0.0};
    // double WN[NX*NX] = {0.0};

    // // Relaxed weights to improve solver convergence
    // W[0] = 5.0;  // X
    // W[1*(NU+NX)+1] = 5.0; // Y
    // W[2*(NU+NX)+2] = 10.0;  // Yaw (reduced to avoid over-penalizing)
    // W[3*(NU+NX)+3] = 10.0; // Linear velocity
    // W[4*(NU+NX)+4] = 10.0;  // Angular velocity (increased to allow more rotation)
    double y_ref[NY] = {0.0};
    y_ref[0] = absolute_goal[0];
    y_ref[1] = absolute_goal[1];
    y_ref[2] = absolute_goal[2];
    y_ref[3] = 1.0; // Encourage positive linear velocity
    y_ref[4] = 0.0;
    ...
    W[0] = 15.0;  // X
    W[1*(NU+NX)+1] = 15.0; // Y
    W[2*(NU+NX)+2] = 15.0;  // Yaw
    W[3*(NU+NX)+3] = 15.0; // Linear velocity (increased to penalize negative velocities)
    W[4*(NU+NX)+4] = 4.6;  // Angular velocity

    WN[0] = 50.0;
    WN[NX+1] = 100.0;
    WN[2*NX+2] = 100.0; // Terminal yaw weight
    // WN[3*NX+3] = 150.0; // Terminal linear velocity (new)
    // WN[4*NX+4] = 130.0; // Terminal angular velocity (new)


    WN[0] = 30.0;
    WN[NX+1] = 30.0;
    WN[2*NX+2] = 20.0; // Terminal yaw weight

    double costmap_resolution = 0.05;
    int costmap_width = 26;
    int costmap_height = 26;
    ...
    double obstacle_weight = 50.0; // Reduced to avoid over-penalizing obstacles
    ...
    // Ensure the initial state exactly matches current_robot_position
    x_init[0] = current_robot_position[0];
    x_init[1] = current_robot_position[1];
    x_init[2] = current_robot_position[2];
    for(int i=1; i<=N; i++) { // Start from i=1 since i=0 is set above
        ...
        double interpolated_yaw = current_robot_position[2] + alpha * yaw_diff; // Use yaw_diff for smoother interpolation
        interpolated_yaw = atan2(sin(interpolated_yaw), cos(interpolated_yaw)); // Normalize yaw
        ...
        if (i * 3 < 30 && tracking_goal[i * 3] != 0.0) {
            ...
            double blend_factor = 0.5; // Reduced blend factor for smoother transition
    ...
    int print_level = 1; // Increased print level for more solver debug info
...
//     W[2*(NU+NX)+2] = 15.0;  // Yaw
//     W[3*(NU+NX)+3] = 15.0; // Linear velocity (increased to penalize negative velocities)
//     W[4*(NU+NX)+4] = 5.0;  // Angular velocity

//     WN[0] = 50.0;
//     WN[NX+1] = 50.0;
//     WN[2*NX+2] = 40.0; // Terminal yaw weight
